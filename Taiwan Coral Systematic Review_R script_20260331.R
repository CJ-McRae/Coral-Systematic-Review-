...
species_data <- as.data.frame(read_csv("sp_list.csv")) 
species_data <- species_data[!species_data$Updated_names=='not_valid',] # remove not valid species

species_data2 <- species_data %>%
  group_by(Updated_names) %>%
  summarise(frequency = sum(Freq, na.rm = TRUE)) %>%
  arrange(desc(frequency))

# numbers of hard and soft coral taxa studied
species_data3 <- left_join(species_data2, species_data, by ='Updated_names')
species_data3 <- species_data3[-c(3:5)]
species_data3 <- species_data3[!duplicated(species_data3), ]
sum(species_data3$Coral_type=='hard_coral')
sum(species_data3$Coral_type=='soft_coral')

# numbers of hard and soft coral taxa identified to species level, and not identified to species level
not_spcies_level <- species_data3[grepl("_sp$|_spp$|dae$", species_data3$Updated_names), ]
spcies_level <- species_data3[!grepl("_sp$|_spp$|dae$", species_data3$Updated_names), ]
sum(spcies_level$Coral_type=='hard_coral')
sum(spcies_level$Coral_type=='soft_coral')

# Top 15 most frequently mentioned hard and soft coral species
hard_coral_top15 <- spcies_level[spcies_level$Coral_type=='hard_coral',]
hard_coral_top15 <- hard_coral_top15[c(1:16),]

soft_coral_top15 <- spcies_level[spcies_level$Coral_type=='soft_coral',]
soft_coral_top15 <- soft_coral_top15[c(1:16),]

# genus trend
genus_data <- data.frame(as.factor(species_data$Genus) , as.numeric(species_data$Freq) )
colnames(genus_data) <- c('genus','frequency')
aggregate(frequency~genus,genus_data,sum)
